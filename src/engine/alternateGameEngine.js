...
export class AlternateGameEngine {
    constructor() {
        this.tiles    = [];
        this.dir      = 0;
        this.score    = 0;
	...
	getOpenPositions = () => this.validPositions.filter(i => this.tiles.map(x => x.pos).indexOf(i) === -1);
	getTile          = pos => this.tiles.filter(x => x.pos === pos)[0];
	removeTile       = tile => this.tiles.splice(this.tiles.indexOf(tile), 1);
	winCondition     = () => this.tiles.some(x => x.val === 2048);
